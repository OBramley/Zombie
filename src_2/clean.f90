MODULE clean
    ...
    subroutine clean_setup(cstore,nume,cleanham,elecs,clean_ndet,zstore)
        ...
        integer::j,k,ierr,total,total2,total3,totalf,checker,pyscfc
        ...
        pyscfc=1
        if(pyscfc.eq.1)then
            call pyscf_clean(cstore, clean_ndet,nume)
        else
    ...
    subroutine pyscf_clean(cstore, clean_ndet,nume)
        ...
        open(unit=204, file=filenm,status='old',iostat=ierr)
        if (ierr.ne.0) then
        ...
        call alloczs(cstore,clean_ndet)
        allocate(combs(clean_ndet,(nume+1)))
        if (ierr.ne.0) then
        ...
        read(204,*, iostat=ierr) ((combs(j,l),l=1,(nume+1)),j=1,clean_ndet)

        close(204)

        do j=1, clean_ndet
            call zomhfc(cstoretemp(j),combs(j,2:))
        end do
