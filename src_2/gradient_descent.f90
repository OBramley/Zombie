MODULE gradient_descent
    ...
    subroutine he_full_row(haml,zstore,elecs,diff_state,size,occupancy_2an,occupancy_an_cr,occupancy_an)
        ...
        do m=1,size
            ...
            call one_elec_intfc(zstore(diff_state),z2l,elecs%h1ei,temp,occupancy_an_cr)

            z2l=z2l*occupancy_an
            !$omp flush(z2l)

            call two_elec_intfc(zstore(diff_state),zstore(m),z2l,z1jk,elecs%h2ei,tot)
            
            ! !!$omp parallel private(j,k,l) shared(z1jk,elecs,occupancy_an_cr,occupancy_2an,occupancy_an,z2l,zstore,temp,tot)
            ! !!$omp do simd collapse(2)
            ! do j=1, norb
            !     do k=1, norb
            !         if(elecs%h1ei(j,k).ne.(0.0)) then 
            !             temp(j,k)=one_elec_body(zstore(diff_state),z2l(j,:,:),occupancy_an_cr(j,k,:,:),elecs%h1ei(j,k),k)
            !         end if
            !     end do
            ! end do
            ! !!$omp end do simd
            ...
            ! !!$omp do
            ! do j=1, norb
            !     tot(j,:,:) = two_elec_part_body(zstore(diff_state),zstore(m),z2l,z1jk(j,:,:,:),elecs%h2ei(j,:,:,:),j)
            ! end do
            ! !!$omp end  do
            ! !!$omp end parallel
    ...
    subroutine one_elec_intfc(zs1,z2l,h1ei,temp,occupancy)
        ...
        !$omp parallel private(j,k) shared(h1ei,occupancy,z2l,temp,zs1)
        !$omp do simd collapse(2)
        do j=1, norb
            do k=1, norb
    ...
    end subroutine one_elec_intfc

    subroutine two_elec_intfc(zs1,zs2,z2l,z1jk,h2ei,tot)
        ...
        !$omp parallel shared(z1jk,z2l,zs1,zs2,tot,h2ei) private(j)
        !$omp do
        do j=1, norb
    ...
    subroutine gradient_row(haml,zstore,elecs,diff_state,size,occupancy_2an,occupancy_an_cr,occupancy_an)
        ...
        do m=1,size
            ...
            call one_elec_intfc_grad(zstore(diff_state),zstore(m),z2l,occupancy_an_cr,elecs%h1ei,equal,h1etot_diff)
      
            z2l=z2l*occupancy_an
            !$omp flush(z2l) 
            
            call two_elec_intfc_grad(zstore(diff_state),zstore(m),z2l,z1jk,elecs%h2ei,occupancy_2an,occupancy_an,equal,h2etot_diff)
    ...
    subroutine one_elec_intfc_grad(zs1,zs2,z2l,occupancy,h1ei,equal,h1etot_diff)
        ...
        !$omp parallel private(j,k) shared(h1ei,occupancy,z2l,zs1,zs2,equal,h1etot_diff)
        !$omp do simd collapse(2)
        do j=1, norb
            do k=1, norb
                if(h1ei(j,k).ne.(0.0)) then 
    ...
    end subroutine one_elec_intfc_grad

    subroutine two_elec_intfc_grad(zs1,zs2,z2l,z1jk,h2ei,occupancy_2an,occupancy_an,equal,h2etot_diff)
        ...
        !$omp parallel shared(z1jk,z2l,zs1,zs2,occupancy_2an,occupancy_an,h2ei,equal,h2etot_diff) private(j)
        !$omp do
        do j=1, norb
    ...
    end subroutine two_elec_intfc_grad
...
!         complex(kind=8), dimension(:,:),intent(inout)::pinv
!         complex(kind=8), dimension(:,:),intent(inout)::pkinvh
!         integer,intent(in)::size,diff_state
!         integer,dimension(:,:,:,:),intent(in)::occupancy_2an,occupancy_an_cr
!         integer,dimension(:,:,:),intent(in)::occupancy_an
...
!         real(kind=8), dimension(:,:,:,:),intent(inout)::pdiff_invh
!         integer,intent(in)::size,diff_state
!         integer,dimension(:,:,:,:),intent(in)::occupancy_2an,occupancy_an_cr
!         integer,dimension(:,:,:),intent(in)::occupancy_an
!         complex(kind=8),dimension(norb,norb,2,norb)::z1jk
...
!             !$omp & shared(ph1ei,psin,pcos,pphi,occupancy_an_cr,z2l,diff_state)&
!             !$omp & private(zomt,temp1,bra_prod,prod)
!             do j=1, norb
!                 do k=1, norb
!                     if(ph1ei(j,k).eq.0.0)then
!                         cycle
...
!             !$omp & shared(ph2ei,psin,pcos,pphi,z2l,z1jk,occupancy_2an,occupancy_an,diff_state)&
!             !$omp & private(gg_1,hh_1,gmax1,hmin1,jspin,occupancy,breakflag,vmultr,vmult_dd,temp,tot)
!             do j=1, norb
!                 if(modulo(j,2)==0)then
!                     jspin=2
!                 else
...
!         integer,dimension(:),intent(inout)::pgrad_avlb
!         type(hamiltonian),intent(inout)::haml
!         type(dvector),dimension(:),intent(inout)::dvec
!         type(energy),intent(inout)::en
!         integer,dimension(:,:,:),intent(in)::occupancy_an
...
!         complex(kind=8), dimension(:,:),intent(inout)::phjk,phjkt
!         complex(kind=8), dimension(:,:),intent(inout)::povrlp,povrlpt
!         complex(kind=8), dimension(:,:),intent(inout)::pinv,pinvt
!         complex(kind=8), dimension(:,:),intent(inout)::pkinvh,pkinvht
!         real(kind=8), dimension(:,:,:),intent(inout)::pdiff_hjk
!         real(kind=8), dimension(:,:,:),intent(inout)::pdiff_ovrlp       
!         real(kind=8), dimension(:,:,:,:),intent(inout)::pdiff_invh
!         real(kind=8),dimension(:,:),intent(inout)::pvars
!         real(kind=8),intent(inout):: pprev_erg
!         real(kind=8),intent(inout):: pcurrent_erg
!         integer,dimension(:),intent(inout)::pgrad_avlb
!         real(kind=8),dimension(:,:),intent(inout)::pprev_mmntm
!         type(dvector),dimension(:),intent(inout)::dvecs,temp_dvecs
!         type(energy),intent(inout)::en
...
!                         temp_ham%hjk=phjkt
!                         temp_ham%ovrlp=povrlpt
!                         temp_ham%kinvh=pkinvht
!                         ! Imaginary time propagation for back tracing
!                         ! print*,'after herow'
!                         en%erg=0
!                         en%t=0
...
!                                 !$omp end target
!                                 !$omp target update from(psin(pick,pickorb),pcos(pick,pickorb),pphi(pick,pickorb),pprev_erg,pinvt)
!                                 temp_ham%inv=pinvt
!                                 haml=temp_ham
!                                 pgrad_avlb=0
!                                 dvecs=temp_dvecs
...
!                             chng_trk(j)=pick
!                             !$omp end target
!                             !$omp target update from(psin,pcos,pphi,pprev_erg,pinvt)
!                             temp_ham%inv=pinvt
!                             haml=temp_ham
!                             pgrad_avlb=0
!                             dvecs=temp_dvecs
...
!         type(zombiest),dimension(:),intent(inout)::zstore
!         type(grad),intent(inout)::grad_fin
!         type(elecintrgl),intent(inout)::elect
!         type(dvector),dimension(:),intent(inout)::dvecs
!         type(dvector), dimension(:), allocatable:: temp_dvecs
!         type(energy),intent(inout)::en
!         type(hamiltonian),intent(inout)::haml
...
!         complex(kind=8), dimension(ndet,ndet)::phjk,phjkt
!         complex(kind=8), dimension(ndet,ndet)::povrlp,povrlpt
!         complex(kind=8), dimension(ndet,ndet)::pinv,pinvt
!         complex(kind=8), dimension(ndet,ndet)::pkinvh,pkinvht
...
!         real(kind=8),dimension(ndet,norb)::pvars
!         real(kind=8):: pprev_erg
!         real(kind=8):: pcurrent_erg
!         integer,dimension(ndet)::pgrad_avlb
!         real(kind=8),dimension(ndet,norb)::pprev_mmntm
!         integer::ierr,j,k,l
