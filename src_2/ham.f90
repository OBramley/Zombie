MODULE ham 
    ...
    ! ! Subroutine to calcualte Hamiltonian elements combines 1st and 2nd electron integral calcualations so remove double 
    ! ! calcualiton of certain results. This minimises the (slow) applicaiton of the creation and annihilaiton operators
    ! subroutine he_row_gpu(ph1ei,ph2ei,phnuc,psin,pcos,pphi,pimg,pdead,palive,occupancy_2an,&
    !     occupancy_an_cr,occupancy_an,passback,phjk,povrlp,pinv,pkinvh,pdiff_hjk,pdiff_ovrlp,pdiff_invh,row,size)
    ...
    !     real(kind=8),dimension(:,:),intent(in)::pimg
    !     integer,dimension(:,:),intent(in)::pdead
    !     integer,dimension(:,:),intent(in)::palive
    ...
    !     !$omp target map(alloc:z1jk(norb,norb,2,norb))
    !     allocate(z1jk(norb,norb,2,norb),stat=ierr)
    !     if(ierr==0) allocate(z2l(norb,2,norb),stat=ierr)
    !     if (ierr/=0) then
    !         write(0,"(a,i0)") "Error in annihilation and creation array vector allocation . ierr had value ", ierr
    ...
    !     !$omp target map(tofrom:h1etot,h2etot,h1etot_diff_bra,h1etot_diff_ket,h2etot_diff_bra,h2etot_diff_ket)
    !     h1etot=cmplx(0.0,0.0)
    !     h2etot=cmplx(0.0,0.0)
    !     h1etot_diff_bra=0.0
    !     h1etot_diff_ket=0.0
    ...
    !         if(m.eq.row)then
    !             call one_elec_part(zstore(row),z2l,h1etot,occupancy_an_cr,&
    !                                 ph1ei,h1etot_diff_bra,h1etot_diff_ket,zstore(m),equal)
    !         else 
    !             call one_elec_part(zstore(row),z2l,h1etot,occupancy_an_cr,&
    !                                 ph1ei,h1etot_diff_bra,h1etot_diff_ket,zstore(m),equal)
    !         end if
    ...
    !         if(m.eq.row)then
    !             call two_elec_part(zstore(row),z1jk,z2l,h2etot,occupancy_2an,occupancy_an,&
    !                                         ph2ei,h2etot_diff_bra,h2etot_diff_ket,zstore(m),equal)
    !         else 
    !             call two_elec_part(zstore(row),z1jk,z2l,h2etot,occupancy_2an,occupancy_an,&
    !                                         ph2ei,h2etot_diff_bra,h2etot_diff_ket,zstore(m),equal)
    !         end if
    !         povrlp(row,m)=overlap(zstore(row),zstore(m))
    ...
    !                 else
    !                     overlap_diff = diff_overlap(zstore(row),zstore(m),equal)
    !                     pdiff_ovrlp(row,m,:) = overlap_diff(1,:)
    !                 end if
    !             else if(m.eq.2)then
    !                 pdiff_hjk(m,row,:)=h1etot_diff_ket+h2etot_diff_ket
    !                 overlap_diff = diff_overlap(zstore(row),zstore(m),equal)
    !                 pdiff_ovrlp(m,row,:) = overlap_diff(2,:)
    !             end if
    !         end if
    ...
    ! subroutine one_elec_part_gpu(zs1,z2l,h1etot,occupancy,h1ei,h1etot_diff_bra,h1etot_diff_ket,zs2,equal)
    ...
    ! subroutine two_elec_part_gpu(zs1,z1jk,z2l,h2etot,occupancy_2an,occupancy_an,h2ei,h2etot_diff_bra,h2etot_diff_ket,zs2,equal)
    ...
    !     real(kind=8),dimension(ndet,norb)::pimg
    !     integer,dimension(ndet,norb)::pdead
    !     integer,dimension(ndet,ndet)::palive
    ...
    !         pphi(j,:)=zstore(j)%phi(:)
    !         pimg(j,:)=zstore(j)%img(:)
    !         pdead(j,:)=zstore(j)%dead(:)
    !         palive(j,:)=zstore(j)%alive(:)
    !     end do

    !     !$omp target data map(to:ph1ei,ph2ei,phnuc,psin,pcos,pphi,pimg,pdead,palive,&
    !     !$omp occupancy_2an,occupancy_an_cr,occupancy_an,passback,phjk,povrlp,pinv,pkinvh,pdiff_hjk,pdiff_ovrlp,pdiff_invh)
    !     do j=1, size
    !         call he_row_gpu(ph1ei,ph2ei,phnuc,psin,pcos,pphi,pimg,pdead,palive,occupancy_2an,&
    !         & occupancy_an_cr,occupancy_an,passback,phjk,povrlp,pinv,pkinvh,pdiff_hjk,pdiff_ovrlp,pdiff_invh,j,size)
    !         if(verb.eq.1)then
    !             write(6,"(a,i0,a)") "Hamiltonian row ",j, " completed"
    !         end if  
    !     end do
