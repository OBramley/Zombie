...
module dnad
    ...
    implicit none
    integer, PARAMETER :: ndv = 4   ! cprev_vars and dcdx_vars

    private

    real(wp) :: negative_one = -1.0
    ...
    type,public :: dual ! make this private will create difficulty to use the
        ...
        real(wp), dimension(:), allocatable:: dx  ! derivative
    end type dual
...
!---------------------
    ...
    elemental subroutine assign_di(u, j)
         ...
         u%x = real(j,kind=wp)  ! This is faster than direct assignment
         u%dx = 0.0
...
!---------------------
    ...
    elemental subroutine assign_dc_i(u, j)
         ...
         u%z = complex(real(j,kind=wp), 0.0)  ! This is faster than direct assignment
...
!
  ...
  elemental function sqrt_dc(u) result(res)
      ...
      if (res%z /= complex(0.0, 0.0)) then
          ...
          do j = 1, ndv
              if (u%dz(j) == complex(0.0, 0.0)) then
                  res%dz(j) = complex(0.0, 0.0)
              else
                  res%dz(j) = set_NaN()
              end if
          end do
...
!
  ...
  ! -----------------------------------------
    elemental function erf_c(c) result(res)
        ...
        real(wp):: x, y
        complex(wp), parameter :: j = complex(0, 1)
        integer :: k
