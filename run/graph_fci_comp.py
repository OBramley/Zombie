...
def plot_energy(eb, filename,c1,c2,comp):
    ...
    ax.axhline(y=comp, color=colors(c2), linewidth=2,label='FCI energy: '+"{:.5f}".format(comp))
    ax.set_xlim(0,beta)
    ax.legend()
...
def plot_energy_gradient_descent(eb, ebf, filename, c1, c2,c3,comp):
    ...
    ax.axhline(y=comp, color=colors(c3), linewidth=2,label='FCI energy: '+"{:.5f}".format(comp))
    ax.set_xlim(0,beta)
    ax.legend()
    ax.set_ylabel('Energy [au]',labelpad=10)
    ax.set_xlabel(r'$\mathregular{\beta}$',labelpad=10)
